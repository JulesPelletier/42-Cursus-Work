...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/09/16 04:54:26 by thhusser          #+#    #+#             */
/*   Updated: 2021/09/16 04:54:26 by thhusser         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	first_display(t_global *g)
{
	...
	move = ft_itoa(g->move);
	mlx_string_put(g->window.mlx_ptr, g->window.win_ptr,
		16, 36, 0xFF000000, move);
	free(move);
	if (g->end == 1)
	{
		ft_putstr("Good Game !\n");
