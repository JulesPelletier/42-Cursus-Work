...
namespace	test_v
{
	...
	test_access(T *init, size_t size_init)
	{
		...
		show_vector(c, "");
		std::cout << "\n";
		std::cout << "\tShowing 1st element \t: " << c[0] << " | " << c.at(0) << "\n";
		std::cout << "\tShowing 4th element \t: " << c[4] << " | " << c.at(4) << "\n";
	...
	test_modifiers(T *init, size_t size_init, T def, T var, size_t size_def)
	{
		...
		show_vector(c2, "COPY");
		std::cout << "\n";
		std::cout << Yellow"\taNAMESPACEer assigning default value : \n" Reset;
		c1.assign(size_def, def);
		show_vector(c1, "ORIGINAL");
		std::cout << Yellow"\taNAMESPACEer pushing back default value : \n" Reset;
		c1.push_back(def);
		show_vector(c1, "ORIGINAL");
		std::cout << Yellow"\taNAMESPACEer double poping back default value : \n" Reset;
		c1.pop_back();
		c1.pop_back();
		show_vector(c1, "ORIGINAL");
		std::cout << Yellow"\taNAMESPACEer inserting var at position 0, 1 and 3: \n" Reset;
		c1.insert(c1.begin(), (size_t)2, var);
		c1.insert(c1.begin() + 3, (size_t)1, var);
		show_vector(c1, "ORIGINAL");
		std::cout << Yellow"\taNAMESPACEer erase first element: \n" Reset;
		c1.erase(c1.begin());
		show_vector(c1, "ORIGINAL");
