...
int	shm_att_pb(render *d,XErrorEvent *ev)
{
...
void	*mlx_int_new_xshm_image(t_xvar *xvar,int width,int height,int format)
{
  ...
  img->data = 0;
  img->image = XShmCreateImage(xvar->render,xvar->visual,xvar->depth,
  ...
  if (!XShmAttach(xvar->render,&(img->shm)) ||
      0&XSync(xvar->render,False) || mlx_X_error)
    {
  ...
  if (xvar->pshm_format==format)
    {
      img->pix = XShmCreatePixmap(xvar->render,xvar->root,img->shm.shmaddr,
				  &(img->shm),width,height,xvar->depth);
      img->type = MLX_TYPE_SHM_PIXMAP;
    }
  else
    {
      img->pix = XCreatePixmap(xvar->render,xvar->root,
...
void	*mlx_int_new_image(t_xvar *xvar,int width, int height,int format)
{
  ...
  img->image = XCreateImage(xvar->render,xvar->visual,xvar->depth,format,0,
  ...
  img->width = width;
  img->height = height;
  img->pix = XCreatePixmap(xvar->render,xvar->root,width,height,xvar->depth);
  img->format = format;
