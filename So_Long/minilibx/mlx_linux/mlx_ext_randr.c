...
int			mlx_ext_fullscreen(t_xvar *xvar, t_win_list *win, int fullscreen)
{
  ...
  if (!XGetWindowAttributes(xvar->render, win->window, &watt))
    return (0);

  res = XRRGetScreenResources(xvar->render, xvar->root);
  o_info = NULL;
  idx_output = -1;
  i = res->noutput;
  while (i--)
    {
      o_info = XRRGetOutputInfo(xvar->render, res, res->outputs[i]);
  ...
  crtc = XRRGetCrtcInfo(xvar->render, res, o_info->crtc);
  saved_mode = crtc->mode;

  i = XRRSetCrtcConfig(xvar->render, res, o_info->crtc, CurrentTime, 0, 0, mode_candidate,
  ...
  XMoveWindow(xvar->render, win->window, 0, 0);
  XMapRaised(xvar->render, win->window);

  if (fullscreen)
    {
      //      XGrabPointer(xvar->render, win->window, True, 0, GrabModeAsync, GrabModeAsync, win->window, 0L, CurrentTime);
      XGrabKeyboard(xvar->render, win->window, False, GrabModeAsync, GrabModeAsync, CurrentTime);
    }
  else
    {
      XUngrabPointer(xvar->render, CurrentTime);
      XUngrabKeyboard(xvar->render, CurrentTime);
    }
