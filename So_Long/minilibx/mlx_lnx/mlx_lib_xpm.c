				     ...
				     int (*xpm_func)(),void *param)
{
  ...
  if (xpm_func(xvar->render,param,&img1,&img2,&xpm_att))
    return ((void *)0);
  if (img2)
    XDestroyImage(img2);
  ...
  *height = img1->height;
  ...
  if (im2->type==MLX_TYPE_SHM_PIXMAP)
    {
      XFreePixmap(xvar->render,im2->pix);
      im2->pix = XCreatePixmap(xvar->render,xvar->root,
			       *width,*height,xvar->depth);
    }
  if (im2->type>MLX_TYPE_XIMAGE)
    {
      XShmDetach(xvar->render,&(im2->shm));
      shmdt(im2->data);
    }
