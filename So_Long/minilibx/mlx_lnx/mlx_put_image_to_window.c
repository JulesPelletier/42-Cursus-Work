				...
				int x,int y)
{
  ...
  if (img->gc)
    {
      gc = img->gc;
      XSetClipOrigin(xvar->render, gc, x, y);
    }
  if (img->type==MLX_TYPE_SHM)
    XShmPutImage(xvar->render,img->pix, win->gc, img->image,0,0,0,0,
		 img->width,img->height,False);
  if (img->type==MLX_TYPE_XIMAGE)
    XPutImage(xvar->render,img->pix, win->gc, img->image,0,0,0,0,
	      img->width,img->height);
  XCopyArea(xvar->render,img->pix,win->window, gc,
